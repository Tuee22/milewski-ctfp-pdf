...
\code{(<>)}\footnote{Original¹⁶: ``\code{mappend}''} inside \code{compose} and \code{mempty} inside
\code{identity} (in place of \code{+} and \code{""}). 
\emph{UPDATE NOTE: Modern Haskell uses \code{(<>)} from \code{Semigroup} instead of \code{mappend}.} There really
